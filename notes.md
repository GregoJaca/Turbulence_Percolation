...
#### Relationship between strong mixing and entropy:

Let the chaotic saddle be partitioned into $N$ small microstates. Initially a trajectory is localized: the probability $P_i$ of finding the state in partition $i$ is $1$, and $0$ for all others. 

$S = - \sum P_i \ln P_i = 0$.

As the positive Lyapunov exponents stretch the trajectory, the strongly mixing flow distributes the probability measure uniformly across all accessible partitions. In the fully mixed state, $P_i = 1/N$ for all $i$.

$$S = - \sum_{i=1}^N \left(\frac{1}{N}\right) \ln\left(\frac{1}{N}\right) = \ln(N)$$

This is the theoretical maximum S.
